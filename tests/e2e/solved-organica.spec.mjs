...
test("@desktop renderiza el HTML original completo y ejecuta sus controles",async({page})=>{
 ...
 const workspaceBox=await page.locator(".workspace").boundingBox();const box=await document.boundingBox();expect(box.x).toBe(workspaceBox.x);expect(box.y).toBeLessThanOrEqual(1);expect(box.width).toBe(workspaceBox.width);expect(box.height).toBeGreaterThanOrEqual(workspaceBox.height-2);
 ...
 const organicIndex=page.locator(".organic-master-index");await expect(organicIndex).toBeVisible();await page.getByRole("button",{name:"Alquenos",exact:true}).click();await expect(page.getByRole("button",{name:"Alquenos",exact:true})).toHaveClass(/active/);await expect.poll(()=>frame.locator("#atlas-alquenos").evaluate(element=>Math.abs(element.getBoundingClientRect().top)<180)).toBe(true);await page.getByRole("button",{name:"Ocultar índice"}).click();await expect(organicIndex).toHaveAttribute("aria-hidden","true");await expect(page.getByRole("button",{name:"Mostrar índice maestro"})).toBeVisible();await page.getByRole("button",{name:"Mostrar índice maestro"}).click();await expect(organicIndex).toHaveAttribute("aria-hidden","false");
...
test("@mobile mantiene utilizable el resumen enriquecido",async({page})=>{
 ...
 await expect(page.locator(".sidebar")).toBeVisible();await expect(page.locator(".organic-master-index")).toBeVisible();const workspaceBox=await page.locator(".workspace").boundingBox();const box=await page.locator(".rich-document").boundingBox();expect(box.x).toBe(workspaceBox.x);expect(box.width).toBe(workspaceBox.width);expect(box.height).toBeGreaterThanOrEqual(workspaceBox.height-2);
});
