...
test("@desktop preselecciona materia, unidad y perfil y persiste o restaura",async({page})=>{
  await ready(page);await page.locator('[data-open="fisica1"]').first().click();await page.locator("#studyUnit").selectOption("resumen-integral");await page.locator("#factoryFromStudy").click();await expect(page.locator('[name="subjectId"]')).toHaveValue("fisica1");await expect(page.locator('[name="unit"]')).toHaveValue("resumen-integral");await expect(page.locator('[name="profile"]')).toHaveValue("physics");
