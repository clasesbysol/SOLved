...
test("@desktop nota compacta, búsqueda, temporizador, lectura y fórmulas persisten",async({page})=>{
  ...
  await expect(page.locator(".note-book")).toHaveCount(1);await page.reload();await page.locator('[data-open="fisica1"]').first().click();await page.locator("#studyUnit").selectOption("demo");await expect(page.locator(".note-book")).toHaveCount(1);
  await page.locator("#globalSearch").fill("mitocondira");await expect(page.locator("#searchResults")).toContainText("Nota personal");
  await page.locator("#timerBtn").click();await page.locator('#timerPanel [data-minutes="15"]').click();expect((await page.evaluate(()=>LBT_DB.getAll("studySessions"))).some(item=>item.status==="running")).toBeTruthy();
  await page.locator("#readingBtn").click();await page.locator('#readingPanel input[name="large"]').check();await expect(page.locator("html")).toHaveClass(/reading-large/);await page.reload();await page.locator('[data-open="fisica1"]').first().click();await expect(page.locator("html")).toHaveClass(/reading-large/);
  await page.getByRole("button",{name:"Fórmulas"}).click();await expect(page.locator(".formula-card").first()).toBeVisible();
});
