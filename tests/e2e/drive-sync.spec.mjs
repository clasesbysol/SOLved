...
async function mockGoogle(page, remoteFiles=[]) {
  ...
  await page.addInitScript(files => {
    window.__driveMock={files,uploads:[],tokensIssued:0,force401:false};
    ...
    window.fetch=async (url,options={})=>{
      ...
      if(download)return new Response(JSON.stringify(download.data),{status:200,headers:{"Content-Type":"application/json"}});
      if(url.includes("/upload/drive/v3/files")){window.__driveMock.uploads.push({url,method:options.method});return new Response(JSON.stringify({id:"canonical"}),{status:200,headers:{"Content-Type":"application/json"}})}
...
test("@desktop conecta, crea appDataFolder y no persiste el token", async ({page}) => {
  ...
  expect(await page.evaluate(()=>window.__driveMock.uploads)).toEqual([{url:expect.stringContaining("uploadType=multipart"),method:"POST"}]);
