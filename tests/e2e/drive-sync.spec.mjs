...
async function accountAction(page,selector){const menu=page.locator("#accountMenu");if(await menu.isHidden())await page.locator("#accountMenuToggle").click();await page.locator(selector).click()}
...
test("@desktop funciona localmente sin Google y conserva cambios offline", async ({page,context}) => {
  ...
  await page.locator('[data-page="subjects"]').click();await page.locator('[data-status="fisica1"]').selectOption("recursando");
  await expect(page.locator("#syncText")).toHaveText("Sin conexión");
  expect(await page.evaluate(()=>window.LBT_DB.get("subjects","fisica1").then(x=>x.status))).toBe("recursando");
  await context.setOffline(false);
});

test("@desktop conecta, crea appDataFolder y no persiste el token", async ({page}) => {
  await mockGoogle(page);await openApp(page);await accountAction(page,"#accountSync");
...
test("@desktop reutiliza cliente y permiso, agrupa autorización y solo revoca al desconectar",async({page})=>{
  ...
  await page.evaluate(()=>window.__driveMock.force401=true);await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Drive pausado · tocar para reconectar");
  await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Sincronizado con Drive");expect(await page.evaluate(()=>({tokens:__driveMock.tokensIssued,clients:__driveMock.tokenClients,prompts:__driveMock.prompts}))).toEqual({tokens:2,clients:1,prompts:["",""]});
  await accountAction(page,"#accountDisconnect");expect(await page.evaluate(()=>__driveMock.revokes.length)).toBe(1);expect(await tokenLeaks(page)).toBe(false);
});

test("@desktop un token vencido pausa Drive sin abrir Google automáticamente",async({page})=>{await mockGoogle(page);await page.addInitScript(()=>{window.addEventListener("DOMContentLoaded",()=>{window.__driveMock.expiresIn=1})});await openApp(page);await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Drive pausado · tocar para reconectar");expect(await page.evaluate(()=>__driveMock.tokensIssued)).toBe(1);await page.locator('[data-page="subjects"]').click();await page.locator('[data-status="fisica1"]').selectOption("recursando");expect(await page.evaluate(()=>__driveMock.tokensIssued)).toBe(1);await page.reload();await page.waitForFunction(()=>typeof document.querySelector("#driveActionBtn")?.onclick==="function");expect(await page.evaluate(()=>__driveMock.tokensIssued)).toBe(0)});
...
test("@desktop descarga y combina progreso, calendario, resaltados y múltiples copias", async ({page}) => {
  ...
  await mockGoogle(page,files);await openApp(page);await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Sincronizado con Drive");
...
test("@desktop conserva cambios ante 401 y reconecta solo mediante botón", async ({page}) => {
  await mockGoogle(page);await openApp(page);await page.evaluate(()=>window.__driveMock.force401=true);await accountAction(page,"#accountSync");
  ...
  await page.locator('[data-page="subjects"]').click();await page.locator('[data-status="fisica1"]').selectOption("recursando");expect(await page.evaluate(()=>window.__driveMock.tokensIssued)).toBe(1);expect(await page.evaluate(()=>LBT_DB.get("subjects","fisica1").then(item=>item.status))).toBe("recursando");
  await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Sincronizado con Drive");
  expect(await page.evaluate(()=>window.__driveMock.tokensIssued)).toBe(2);
  const body=await page.evaluate(()=>window.__driveMock.uploads.at(-1).body);expect(body).toContain('"status":"recursando"');
});
...
test("@desktop rechaza remoto corrupto sin modificar IndexedDB", async ({page}) => {
  ...
  await mockGoogle(page,bad);await openApp(page);await page.locator('[data-page="subjects"]').click();await expect(page.locator('[data-status="fisica1"]')).toBeVisible();const before=await page.evaluate(()=>LBT_DB.get("subjects","fisica1"));await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Error de sincronización");expect(await page.evaluate(()=>LBT_DB.get("subjects","fisica1"))).toEqual(before);
});
...
test("@desktop conserva la restauración autoritativa hasta una subida real", async ({page}) => {
  ...
  await accountAction(page,"#accountBackup");await page.locator("#importReplaceBtn").click();await page.locator("#restoreInput").setInputFiles({name:"respaldo.json",mimeType:"application/json",buffer:Buffer.from(JSON.stringify(backup))});
  await expect(page.locator(".toast")).toHaveText("Respaldo restaurado localmente");await accountAction(page,"#accountBackup");await expect(page.locator("#replaceDriveBtn")).toBeVisible();
  await page.reload();await page.waitForFunction(()=>typeof document.querySelector("#driveActionBtn")?.onclick==="function");await accountAction(page,"#accountBackup");await expect(page.locator("#replaceDriveBtn")).toBeVisible();expect(await page.evaluate(()=>LBT_DB.get("subjects","fisica1").then(x=>x.progress))).toBe(88);
  ...
  await page.locator('#backupModal [data-close="backupModal"]').click();await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Cambios pendientes");await accountAction(page,"#accountBackup");await expect(page.locator("#replaceDriveBtn")).toBeVisible();
  ...
  await page.reload();await accountAction(page,"#accountBackup");await expect(page.locator("#replaceDriveBtn")).toBeHidden();
});

test("@mobile conecta Drive sin almacenar credenciales", async ({page}) => {
  await mockGoogle(page);await openApp(page);await accountAction(page,"#accountSync");await expect(page.locator("#syncText")).toHaveText("Sincronizado con Drive");expect(await tokenLeaks(page)).toBe(false);
});
