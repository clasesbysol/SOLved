...
try{
...

  {
    ...
    const result=await page.evaluate(()=>({
      ...
      glucidosIndex:document.querySelectorAll('[data-qbi-glucidos-index="1"]').length,
    ...
    if(result.glucidos!==13||result.glucidosIndex!==13||!result.cap41)throw new Error(`Glúcidos I incompleto: capítulos=${result.glucidos}, índice=${result.glucidosIndex}, cap41=${result.cap41}`);
    const before=new URL(page.url()).pathname;
    await page.click('a[href="#cap20"]');
    await page.waitForTimeout(300);
    const after20=new URL(page.url()).pathname;
    await page.click('a[href="#cap41"]');
    await page.waitForTimeout(300);
