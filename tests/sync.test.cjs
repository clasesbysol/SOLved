...

{
  ...
  assert.equal(chooseRecord(a,b).progress,20,"gana el registro mÃ¡s nuevo");
...
assert.throws(()=>validateEnvelope({}),/versiÃ³n/,"rechaza JSON corrupto");
assert.throws(()=>validateEnvelope(env({schemaVersion:2})),/compatible/,"rechaza esquema desconocido");
assert.doesNotThrow(()=>mergeEnvelopes(env(),env({sourceDeviceId:"b"}),env({sourceDeviceId:"c"})),"combina mÃºltiples archivos");
{
...
(async()=>{
  ...
  await assert.rejects(()=>sync.replaceRemote(),/ReconectÃ¡/,"sin token no informa un reemplazo exitoso");assert.equal(uploads,0);
  ...
  const reloaded=new DriveSync({DB,appVersion:"0.4.5",contentVersion:"demo-2",fetcher,onState:state=>states.push(state)});await reloaded.init();assert.equal(reloaded.isAuthoritativePending(),true,"recupera el bloqueo despuÃ©s de recargar");
  ...
  assert.equal(await reloaded.replaceRemote(),true);assert.equal(uploads,1,"la confirmaciÃ³n autoritativa reemplaza Drive");assert.equal(downloads,0,"el reemplazo no descarga ni mezcla el remoto viejo");
  ...
  console.log("Pruebas de sincronizaciÃ³n y merge: OK");
