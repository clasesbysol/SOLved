...
const FORMULAS=['Geometría tetraédrica del C sp³ ≈ 109,5°','N(estereoisómeros) = 2ⁿ','n = número de centros quirales','β-D-glucopiranosa ≈ 63,6 %','α-D-glucopiranosa ≈ 36,4 %','forma abierta ≈ 0,003 %','Maltosa = Glc α(1→4) Glc · reductora','Lactosa = Gal β(1→4) Glc · reductora','Sacarosa = Glc α(1→2)β Fru · no reductora','Trehalosa = Glc α(1→1)α Glc · no reductora','Amilosa = [Glc α(1→4)]ₙ','Amilopectina = α(1→4) + ramas α(1→6) cada ~24–30 residuos','Glucógeno = α(1→4) + ramas α(1→6) cada ~8–12 residuos','Celulosa = [Glc β(1→4)]ₙ','Quitina = [GlcNAc β(1→4)]ₙ'];
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function sidebar(){return document.getElementById('summaryIndex')||document.querySelector('.summary-index.qb-index')||document.querySelector('.qb-sidebar .qb-index')||document.querySelector('.qb-index')}
...
function installFragmentGuard(){if(document.documentElement.dataset.qbiFragmentGuard)return;document.documentElement.dataset.qbiFragmentGuard='1';document.addEventListener('click',e=>{const a=e.target.closest?.('a[href^="#"]');if(!a)return;const href=a.getAttribute('href');if(!href||href==='#')return;const target=document.getElementById(decodeURIComponent(href.slice(1)));if(!target)return;e.preventDefault();try{history.replaceState(null,'',location.pathname+location.search+href)}catch{}target.scrollIntoView({behavior:'smooth',block:'start'})},true)}
async function load(){const here=new URL(location.href);here.hash='';here.search='';const parts=await Promise.all(FILES.map(name=>fetch(new URL(name,here).href+'?v='+VERSION,{cache:'reload'}).then(r=>{if(!r.ok)throw Error(name+' '+r.status);return r.text()})));return parse(parts.join('\n'))}
function indexReady(){const nav=sidebar();if(!nav)return false;const links=[...nav.querySelectorAll('[data-qbi-glucidos-index="1"]')];return links.length===13&&links.every((a,i)=>a.getAttribute('href')==='#cap'+(29+i))}
function addIndex(){const nav=sidebar();if(!nav)return false;if(indexReady())return true;nav.querySelectorAll('[data-qbi-glucidos-index="1"]').forEach(x=>x.remove());let cursor=nav.querySelector('a[href="#cap28"]');if(!cursor)return false;TITLES.forEach((t,i)=>{const n=29+i,a=cursor.cloneNode(false);a.removeAttribute('id');a.classList.remove('active');a.removeAttribute('style');a.dataset.qbiGlucidosIndex='1';a.href='#cap'+n;a.textContent=`${n}. ${t}`;cursor.insertAdjacentElement('afterend',a);cursor=a});return indexReady()}
function chaptersReady(){return TITLES.every((_,i)=>document.getElementById('cap'+(29+i)))}
function build(parsed){const anchor=document.getElementById('cap28');if(!anchor?.parentNode)return false;if(chaptersReady())return true;for(let n=29;n<=41;n++)document.getElementById('cap'+n)?.remove();let cursor=anchor;TITLES.forEach((t,i)=>{const n=29+i,s=document.createElement('section');s.id='cap'+n;s.className='qb-chapter qbi-glu-native-chapter';s.dataset.qbiGlucidosChapter=String(i+1);s.innerHTML=`<div class="qb-chapter-number">${n}</div><h2>${esc(t)}</h2>${render(parsed.chapters[i]||[],i,parsed.preface)}`;cursor.insertAdjacentElement('afterend',s);cursor=s});return chaptersReady()}
function updateCounters(){const badge=[...document.querySelectorAll('.qb-hero-badges span')].find(x=>/cap[ií]tulos/i.test(x.textContent));if(badge)badge.textContent='41 capítulos';const map=document.querySelector('#qbi-mapa-integral [data-map-count]');if(map&&/cap[ií]tulos/i.test(map.textContent))map.textContent=map.textContent.replace(/cap[ií]tulos/i,'temas del mapa')}
let parsedCache=null,running=false,tries=0;
async function ensure(){if(running)return false;if(!document.getElementById('cap28'))return false;running=true;try{ensureStyle();installFragmentGuard();parsedCache ||= await load();if(!build(parsedCache))return false;if(!addIndex())return false;updateCounters();document.documentElement.dataset.qbiGlucidosAudit='complete';document.documentElement.dataset.qbiGlucidosForce=VERSION;window.QBI_MATH_RENDER_FIX?.repair?.();return true}catch(e){console.error('QBI Glúcidos force '+VERSION,e);return false}finally{running=false}}
async function boot(){if(await ensure())return;if(++tries<180)setTimeout(boot,250)}
function maintain(){if(!chaptersReady()||!indexReady())ensure();else updateCounters()}
const observer=new MutationObserver(()=>{clearTimeout(observer.timer);observer.timer=setTimeout(maintain,80)});
function start(){observer.observe(document.documentElement,{subtree:true,childList:true});boot();setTimeout(maintain,1200);setTimeout(maintain,3200);setInterval(maintain,2500)}
