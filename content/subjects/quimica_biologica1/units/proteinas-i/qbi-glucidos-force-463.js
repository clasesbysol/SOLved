...
let done=false,tries=0;async function run(){if(done)return true;if(!document.getElementById('cap28')||!sidebar())return false;try{ensureStyle();installFragmentGuard();const parsed=await load();if(!build(parsed)||!addIndex())return false;document.documentElement.dataset.qbiGlucidosAudit='complete';document.documentElement.dataset.qbiGlucidosForce='4.6.3';window.QBI_MATH_RENDER_FIX?.repair?.();done=true;return true}catch(e){console.error('QBI Glúcidos force 4.6.3',e);return false}}
async function boot(){if(await run())return;if(++tries<160)setTimeout(boot,250)}
