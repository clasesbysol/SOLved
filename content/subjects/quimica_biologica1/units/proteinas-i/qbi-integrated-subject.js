(()=>{
  ...
  const css=`
  #${MAP_ID}{scroll-margin-top:18px;margin:0 0 28px;padding:clamp(16px,2.4vw,30px);border:1px solid color-mix(in srgb,var(--qb-accent,#e74888) 27%,#ddd);border-radius:22px;background:linear-gradient(145deg,color-mix(in srgb,var(--qb-accent-soft,#fff0f6) 68%,#fff),#fff);box-shadow:0 12px 34px rgba(104,42,75,.08)}
  .qbi-integrated-map-head span{font-size:.76rem;font-weight:900;letter-spacing:.09em;color:var(--qb-accent-strong,#9d285b)}.qbi-integrated-map-head h2{margin:5px 0 7px;font-size:clamp(1.55rem,3vw,2.35rem)}.qbi-integrated-map-head p{max-width:850px;margin:0;color:var(--qb-muted,#75596a)}
  ...
  function renderMap(){
    ...
    section.innerHTML=`<header class="qbi-integrated-map-head"><span>MAPA MENTAL INTEGRAL</span><h2>Química Biológica de un vistazo</h2><p>El mismo mapa mental de SOLved, ahora dentro de la materia. Cada concepto contiene su explicación; el resumen queda inmediatamente debajo para profundizar.</p></header><div class="qbi-integrated-map-tools"><input type="search" data-map-filter placeholder="Buscar dentro del mapa…"><button type="button" data-map-expand>Abrir todo</button><button type="button" data-map-collapse>Cerrar todo</button><output data-map-count></output></div>${data.map((group,index)=>`<section class="qbi-integrated-group"><header><b>${index+1}</b><div><h3>${safe(group.label)}</h3><p>${safe(group.description)}</p></div></header>${group.chapters.map(chapterHtml).join('')}</section>`).join('')}`;
  ...
  function installNotes(side,root){
    ...
    add.onclick=()=>placeMode(!placing);box.querySelector('[data-highlight]').onclick=()=>{root.querySelector('#qbHighlightButton')?.click();help.textContent='Seleccioná texto y volvé a tocar Resaltar para guardarlo.'};
