(()=>{
  ...
  const target={summary:'qb-top',exercises:'ejercicios-estadistica',map:'stats-guide-memory-maps'};
  API.render=(subjectId,unitId,tab)=>{
    if(subjectId===SUBJECT&&unitId===UNIT&&target[tab]){
      const record=API.getUnit(subjectId,unitId),version=record?.contentVersion||'1.4.0',base=record?.path||`content/subjects/${SUBJECT}/units/${UNIT}/`;
