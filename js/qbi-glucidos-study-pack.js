(()=>{
  ...
  function mathify(html){return String(html||'').replace(/<p>\s*\[([\s\S]*?)\]\s*<\/p>/g,(m,tex)=>`<div class="qbi-hub-math">\\[${tex.trim()}\\]</div>`)}
  ...
  function splitProtein(cards){const study=[],practice=[];for(const card of cards||[]){const label=compact(card.guide).toLocaleLowerCase('es');if(label.includes('problemas')||label.includes('electroforesis')||label.includes('2025'))practice.push(card);else study.push(card)}return {study:study.length?study:(cards||[]),practice}}
  ...
  function patchContent(){
    ...
    api.render=(subjectId,unitId,tab)=>{if(subjectId==='quimica_biologica1'&&tab==='exercises')return `<div class="content-card zoom-target" data-qbi-exercises-hub><div class="qbi-hub-loading">Cargando guías de Química Biológica…</div></div>`;return originalRender(subjectId,unitId,tab)};
    ...
    api.__qbiGlucidosStudyPack=VERSION;return true
  }

  function tp3Html(){return `<section id="${TP3_ID}" class="qb-chapter qbi-tp3" data-qbi-tp3-version="${VERSION}"><div class="qb-chapter-kicker">TP3 · GLÚCIDOS</div><h2>TP3 · Métodos de glúcidos y cómo se calculan</h2><p class="qbi-tp3-lead">Este bloque junta los cuatro métodos de la clase de problemas: <strong>Somogyi–Nelson</strong>, <strong>Roe/Seliwanoff</strong>, <strong>metilación exhaustiva + hidrólisis ácida</strong> y <strong>oxidación con periodato</strong>. La idea no es memorizar nombres: es saber <em>qué dato entrega cada técnica</em> y cómo convertir ese dato en concentración, número de unidades o estructura.</p>
  ...
  function injectTP3(doc){
    ...
    const anchor=doc.getElementById('cap38')||doc.getElementById('cap41')||doc.querySelector('.qb-document section:last-of-type');if(!anchor)return;
    ...
    if(nav&&!nav.querySelector(`a[href="#${TP3_ID}"]`)){const ref=nav.querySelector('a[href="#cap38"]')||nav.lastElementChild;const link=doc.createElement('a');link.href=`#${TP3_ID}`;link.textContent='TP3 · Métodos y cálculos de glúcidos';link.dataset.qbiTp3Index='1';if(ref)ref.insertAdjacentElement('afterend',link);else nav.append(link)}
    const target=doc.getElementById(TP3_ID),mj=doc.defaultView?.MathJax;if(mj?.typesetPromise){try{mj.typesetClear?.([target]);mj.typesetPromise([target]).catch(()=>{})}catch(_){}}
  }
  function isQbiFrame(frame){const src=frame.getAttribute('src')||'',title=frame.getAttribute('title')||'';return /quimica_biologica1\/units\/proteinas-i\/(?:qbi-static|original)\.html/i.test(src)||/Química Biológica/i.test(title)}
