(function(root,factory){
  ...
  function multipart(envelope,metadata){const form=new FormData();form.append("metadata",new Blob([JSON.stringify(metadata)],{type:"application/json"}));form.append("file",new Blob([JSON.stringify(envelope)],{type:"application/json"}));return form}
  ...
  class DriveSync{
    ...
    async init(){let item=await this.DB.get("meta","drive-device-id");if(!item){item={key:"drive-device-id",value:crypto.randomUUID(),updatedAt:new Date().toISOString()};await this.DB.put("meta",item)}this.deviceId=item.value;this.onState("disconnected")}
    async requestToken(){
      if(!window.google?.accounts?.oauth2)throw new Error("Google Identity Services no pudo cargarse.");
      return new Promise((resolve,reject)=>{const client=google.accounts.oauth2.initTokenClient({client_id:CLIENT_ID,scope:SCOPE,callback:async response=>{if(response.error){reject(new Error("Google no autorizó la conexión."));return}this.token=response.access_token;this.expiresAt=Date.now()+Number(response.expires_in||3600)*1000;this.needsReconnect=false;this.blockedAfterReplace=false;try{await this.syncNow();resolve()}catch(error){reject(error)}}});client.requestAccessToken({prompt:"consent"})})
    }
    ...
    markLocalReplace(){this.blockedAfterReplace=true;this.onState("pending-authoritative")}
    ...
    async upload(envelope,id=null){const url=id?`https://www.googleapis.com/upload/drive/v3/files/${encodeURIComponent(id)}?uploadType=multipart`:`https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart`;const metadata=id?{name:FILE_NAME}:{name:FILE_NAME,parents:["appDataFolder"],mimeType:"application/json"};await driveFetch(this.fetcher,this.token,url,{method:id?"PATCH":"POST",body:multipart(envelope,metadata)})}
    ...
    async replaceRemote(){if(!this.hasToken()){this.onState("reconnect");return}this.onState("syncing");const local=validateEnvelope(await this.snapshot());const files=await this.listFiles();const canonical=[...files].sort((a,b)=>time(b.modifiedTime)-time(a.modifiedTime)||String(a.id).localeCompare(String(b.id)))[0];await this.upload(local,canonical?.id||null);this.blockedAfterReplace=false;this.onState("synced")}
  }
  return {CLIENT_ID,SCOPE,FILE_NAME,SETTINGS_FIELDS,stable,chooseRecord,mergeRecords,normalizeSettings,mergeSettings,validateEnvelope,mergeEnvelopes,DriveSync};
});
