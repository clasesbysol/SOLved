(function(){
  ...
  function sendRichFrameState(frame=richFrame()){frame?.contentWindow?.postMessage({type:"solved-rich-state",hue:subjectHue(),theme:settings.theme||"light",highlights:richHighlights()},"*")}
  ...
  function bindEvents(){
    ...
    window.addEventListener("message",event=>{const frame=richFrame();if(!frame||event.source!==frame.contentWindow)return;const data=event.data||{};if(data.type==="solved-rich-selection"&&data.piece){pendingSelection=[data.piece];activeHighlightId=null;updateHighlightButton()}if(data.type==="solved-rich-highlight-active"){activeHighlightId=data.id||null;pendingSelection=null;updateHighlightButton()}});
