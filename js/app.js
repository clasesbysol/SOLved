(function(){
  ...
  async function renderStudy(){
    ...
    window.LBT_CONTENT.bind(els.studyBody);const richFrame=els.studyBody.querySelector(".rich-document");if(richFrame)richFrame.addEventListener("load",()=>sendRichFrameState(richFrame));els.studyBody.querySelectorAll("[data-open-factory-context]").forEach(button=>button.onclick=()=>openFactory({subjectId:currentSubject.id,unitId:els.studyUnit.value}));window.LBT_NOTES.render();window.LBT_UTILS?.applyReading(currentSubject?.id);
