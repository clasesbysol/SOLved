(function(){
  ...
  const els={
    pageTitle:$("pageTitle"),crumb:$("crumb"),globalSearch:$("globalSearch"),backupBtn:$("backupBtn"),themeBtn:$("themeBtn"),restoreInput:$("restoreInput"),
    handList:$("handList"),manageCourses:$("manageCourses"),dashboardPage:$("dashboardPage"),subjectsPage:$("subjectsPage"),calendarPage:$("calendarPage"),studyPage:$("studyPage"),
    ...
    studyToolbar:$("studyToolbar"),highlightBtn:$("highlightBtn"),highlightLabel:$("highlightLabel"),selectionHelp:$("selectionHelp"),newNoteBtn:$("newNoteBtn"),contentUpdateBtn:$("contentUpdateBtn"),indexBtn:$("indexBtn"),zoomBtn:$("zoomBtn"),viewerBtn:$("viewerBtn"),
  ...
  function renderHandList(){els.handList.innerHTML=orderedCurrent().map(s=>`<button class="course-link" data-open="${s.id}" style="--hue:${s.hue}"><span class="dot"></span><span class="label"><strong>${safe(s.name)}</strong><small>${safe(STATUS[getSubjectState(s.id).status])}</small></span></button>`).join("");els.handList.querySelectorAll("[data-open]").forEach(b=>b.onclick=()=>openSubject(b.dataset.open))}
  ...
  function refreshUnitSelector(subjectId){const units=window.LBT_CONTENT.units(subjectId),preferred=settings.lastUnitBySubject?.[subjectId],selected=units.some(unit=>unit.unitId===preferred)?preferred:units[0]?.unitId||"legacy";els.studyUnit.innerHTML=units.length?units.map(unit=>`<option value="${safe(unit.unitId)}">${safe(unit.title)}</option>`).join(""):'<option value="legacy">Sin unidades publicadas</option>';els.studyUnit.value=selected;els.studyUnit.disabled=!units.length;return selected}
  async function openSubject(id){currentSubject=subject(id);if(!currentSubject)return;settings.lastSubject=id;settings.lastTab=currentTab="summary";settings.lastUnitBySubject={...(settings.lastUnitBySubject||{})};settings.lastUnitBySubject[id]=refreshUnitSelector(id);await saveSettings();els.studyTitle.textContent=currentSubject.name;els.studyMeta.textContent=subjectMeta(currentSubject)||`${currentSubject.term}.º cuatrimestre`;els.subjectMark.style.setProperty("--hue",currentSubject.hue);els.studyPage.style.setProperty("--hue",currentSubject.hue);els.studyStatus.innerHTML=statusOptions(getSubjectState(id).status);els.studyTabs.querySelectorAll(".tab").forEach(tab=>tab.classList.toggle("active",tab.dataset.tab==="summary"));renderStudy();setPage("study")}
  function renderStudy(){
    const s=currentSubject,unitId=els.studyUnit.value,block=unitId==="legacy"?"Vista integral":unitId;activeHighlightId=null;pendingSelection=null;updateHighlightButton();
    ...
    const dynamic=window.LBT_CONTENT.render(s.id,unitId,currentTab);if(dynamic)tabContent[currentTab]=dynamic;
    ...
    window.LBT_CONTENT.bind(els.studyBody);window.LBT_NOTES.render();
  ...
  async function highlightAction(){
    ...
    const stamp=nowISO();
    for(const piece of pendingSelection){const h={id:uuid(),subjectId:currentSubject.id,unitId:els.studyUnit.value==="legacy"?null:els.studyUnit.value,studyBlock:els.studyUnit.value,tab:"summary",...piece,color:"yellow",contentVersion:CONTENT_VERSION,createdAt:stamp,updatedAt:stamp};highlights.push(h);await persist("highlights",h)}
  ...
  function applyAllHighlights(){
    document.querySelectorAll(".highlightable").forEach(block=>{
      ...
      const legacyBlockId=block.dataset.localBlockId==="demo-intro"?`${currentSubject.id}:Vista integral:intro`:null;const records=highlights.filter(h=>!h.deletedAt&&h.subjectId===currentSubject.id&&(h.blockId===block.dataset.blockId||(!h.unitId&&legacyBlockId&&h.blockId===legacyBlockId))).map(h=>({h,r:resolveRange(h,base)})).filter(x=>x.r).sort((a,b)=>a.r.start-b.r.start);
      block.textContent="";let pos=0;
      for(const {h,r} of records){if(r.start<pos)continue;block.append(document.createTextNode(base.slice(pos,r.start)));const mark=document.createElement("mark");mark.className="study-highlight";mark.dataset.highlightId=h.id;mark.textContent=base.slice(r.start,r.end);if(h.id===activeHighlightId)mark.classList.add("active");mark.onclick=e=>{e.stopPropagation();activeHighlightId=h.id;pendingSelection=null;window.getSelection()?.removeAllRanges();document.querySelectorAll(".selection-ready").forEach(b=>b.classList.remove("selection-ready"));document.querySelectorAll("mark.study-highlight").forEach(m=>m.classList.toggle("active",m.dataset.highlightId===h.id));updateHighlightButton()};block.append(mark);pos=r.end}
      block.append(document.createTextNode(base.slice(pos)));
    });
  }
  ...
  function bindEvents(){
    ...
    [els.eventModal,els.coursesModal,els.backupModal,els.correlationModal].forEach(m=>m.onclick=e=>{if(e.target===m)m.hidden=true});els.manageCourses.onclick=els.manageCoursesPlan.onclick=openCoursesModal;els.addEventTop.onclick=els.addEventSide.onclick=els.addEventCalendar.onclick=()=>openEventModal();
    ...
    els.coursesForm.onsubmit=async e=>{e.preventDefault();const ids=[...els.courseChecks.querySelectorAll("input:checked")].map(x=>x.value);settings.currentIds=ids;settings.order=[...settings.order.filter(id=>ids.includes(id)),...ids.filter(id=>!settings.order.includes(id))];await saveSettings();els.coursesModal.hidden=true;renderDashboard();toast("Materias a mano actualizadas")};
