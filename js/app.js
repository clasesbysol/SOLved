(function(){
  ...
  const els={
    ...
    handList:$("handList"),manageCourses:$("manageCourses"),dashboardPage:$("dashboardPage"),subjectsPage:$("subjectsPage"),calendarPage:$("calendarPage"),favoritesPage:$("favoritesPage"),reviewPage:$("reviewPage"),studyPage:$("studyPage"),
  ...
  function setPage(page){
    ["dashboard","subjects","calendar","favorites","review","study"].forEach(p=>{const el=$(p+"Page");if(el)el.hidden=p!==page});
    document.querySelectorAll(".nav-btn").forEach(b=>b.classList.toggle("active",b.dataset.page===page));
    const titles={dashboard:["Vista general","Licenciatura en Biotecnología · plan completo"],subjects:["Plan y materias","Plan completo y correlatividades"],calendar:["Calendario","Fechas importantes por materia"],favorites:["Favoritos","Colecciones personales"],review:["Repaso aleatorio","Contenido publicado"],study:[currentSubject?.name||"Materia","Biblioteca de estudio"]};
  ...
  function applyAllHighlights(){
    const blocks=[...document.querySelectorAll(".highlightable")];for(const block of blocks){block.__baseText=block.__baseText||block.textContent;block.textContent=block.__baseText}const unit=window.LBT_CONTENT.getUnit(currentSubject.id,els.studyUnit.value),legacyMap={intro:"demo-intro",purpose:"purpose",coverage:"coverage",progress:"progress"},retired=h=>h.unitId&&!window.LBT_CONTENT.getUnit(currentSubject.id,h.unitId),eligible=highlights.filter(h=>!h.deletedAt&&h.subjectId===currentSubject.id&&(h.unitId===unit?.unitId||(!h.unitId&&unit?.unitId==="demo")||retired(h))),resolved=[],orphans=[];
  ...
  async function exportBackup(){const payload=await DB.exportAll(),blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`biblioteca-lbt-respaldo-v051-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);toast("Respaldo completo exportado")}
  ...
  function bindEvents(){
    ...
    els.themeBtn.onclick=async()=>{settings.theme=settings.theme==="light"?"dark":"light";applyTheme();await saveSettings()};
