(function(){
  ...
  function refreshUnitSelector(subjectId){if(subjectId&&!settings.currentIds.includes(subjectId))settings.recentSubjectIds=[subjectId,...(settings.recentSubjectIds||[]).filter(id=>id!==subjectId&&!settings.currentIds.includes(id))].slice(0,3);const units=window.LBT_CONTENT.units(subjectId),migrationKey=`solved-fisica-integral-v1:${DB.dbName||"profile"}`,introduceIntegral=subjectId==="fisica1"&&!localStorage.getItem(migrationKey)&&units.some(unit=>unit.unitId==="resumen-integral"),preferred=introduceIntegral?"resumen-integral":settings.lastUnitBySubject?.[subjectId],selected=units.some(unit=>unit.unitId===preferred)?preferred:units[0]?.unitId||"legacy";if(introduceIntegral)localStorage.setItem(migrationKey,"1");els.studyUnit.innerHTML=units.length?units.map(unit=>`<option value="${safe(unit.unitId)}">${safe(unit.title)}</option>`).join(""):'<option value="legacy">Sin unidades publicadas</option>';els.studyUnit.value=selected;els.studyUnit.disabled=!units.length;return selected}
