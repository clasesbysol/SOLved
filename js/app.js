(function(){
  ...
  const els={
    ...
    handList:$("handList"),manageCourses:$("manageCourses"),dashboardPage:$("dashboardPage"),subjectsPage:$("subjectsPage"),calendarPage:$("calendarPage"),favoritesPage:$("favoritesPage"),reviewPage:$("reviewPage"),factoryPage:$("factoryPage"),studyPage:$("studyPage"),
    ...
    fullscreenBtn:$("fullscreenBtn"),fullscreenExit:$("fullscreenExit"),factoryFromStudy:$("factoryFromStudy"),previewWarning:$("previewWarning"),calendarLegend:$("calendarLegend"),correlationModal:$("correlationModal"),correlationTitle:$("correlationTitle"),correlationContent:$("correlationContent")
  };
  ...
  function renderStudy(){
    ...
    const empty=`<div class="content-card zoom-target empty-study"><div class="eyebrow">${safe(block)}</div><h2>${safe(s.name)}</h2><div class="viewer-empty">${icon("i-file")}<div><strong>Materia creada, todavía vacía</strong><br><span>Acá se cargarán el resumen, glosario, tarjetas, ejercicios y mapa mental cuando empieces esta materia.</span></div></div><button class="primary-btn" data-open-factory-context aria-label="Abrir fábrica de estudio con esta materia">Fabricar resumen con mis documentos</button></div>`;
  ...
  async function exportBackup(){const payload=await DB.exportAll(),blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`biblioteca-lbt-respaldo-v053-${new Date().toISOString().slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);toast("Respaldo completo exportado")}
