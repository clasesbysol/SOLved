(function(){
  ...
  const els={
    ...
    handList:$("handList"),recentSubjectsSection:$("recentSubjectsSection"),recentSubjectsList:$("recentSubjectsList"),manageCourses:$("manageCourses"),dashboardPage:$("dashboardPage"),subjectsPage:$("subjectsPage"),calendarPage:$("calendarPage"),settingsPage:$("settingsPage"),studyPage:$("studyPage"),
    currentCount:$("currentCount"),currentGrid:$("currentGrid"),termSummary:$("termSummary"),planGrid:$("planGrid"),checklistForm:$("checklistForm"),checklistInput:$("checklistInput"),checklistList:$("checklistList"),checklistCount:$("checklistCount"),settingsProfileName:$("settingsProfileName"),settingsProfileEmail:$("settingsProfileEmail"),settingsProfileStatus:$("settingsProfileStatus"),
  ...
  async function initialize(){
    ...
    await window.SOLVED_CLOUD?.init(DB,async()=>{await loadData();renderDashboard();renderPlan();renderCalendar()}).catch(error=>{console.error(error);toast("No se pudo sincronizar con Supabase; tus datos locales siguen disponibles")});
    await migrateHistoricalOwner();await migrateV03();await loadData();driveSync=new window.LBT_SYNC.DriveSync({DB,appVersion:APP_VERSION,contentVersion:CONTENT_VERSION,onState:updateDriveState,onApplied:async()=>{await loadData();await window.LBT_NOTES?.reload();applyTheme();applyStudyPreferences();renderDashboard();renderPlan();renderCalendar();renderSettings()}});await driveSync.init();await window.LBT_CONTENT.init(DB,()=>{if(currentSubject)refreshUnitSelector(currentSubject.id);toast("Contenido actualizado; se aplicará cuando vuelvas a abrir la sección")});const studyContext=()=>{const unit=window.LBT_CONTENT.getUnit(currentSubject?.id,els.studyUnit?.value);return {subjectId:currentSubject?.id,unitId:unit?.unitId||"legacy",tab:currentTab,contentVersion:unit?.contentVersion||"legacy"}};await window.LBT_NOTES.init(DB,driveSync,studyContext);await window.LBT_UTILS.init(DB,driveSync,studyContext,()=>SUBJECTS);els.replaceDriveBtn.hidden=!driveSync.isAuthoritativePending();bindEvents();applyTheme();applyStudyPreferences();renderDashboard();renderPlan();renderCalendar();renderSettings();
  ...
  function setPage(page){
    ...
    els.newNoteBtn.hidden=page!=="study";if(page==="dashboard")renderDashboard();if(page==="subjects")renderPlan();if(page==="calendar")renderCalendar();if(page==="settings")renderSettings();
  ...
  function renderChecklist(){
    const items=settings.checklist||[],pending=items.filter(item=>!item.done).length;els.checklistCount.textContent=String(pending);
    els.checklistList.innerHTML=items.length?items.map(item=>`<div class="checklist-item ${item.done?"done":""}" data-checklist-id="${item.id}"><label><input type="checkbox" ${item.done?"checked":""}><span>${safe(item.text)}</span></label><button type="button" class="event-delete" title="Eliminar tarea" aria-label="Eliminar ${safe(item.text)}">${icon("i-trash")}</button></div>`).join(""):'<div class="empty-box">No hay tareas pendientes.</div>';
    els.checklistList.querySelectorAll("[data-checklist-id]").forEach(row=>{const id=row.dataset.checklistId;row.querySelector("input").onchange=async event=>{settings.checklist=items.map(item=>item.id===id?{...item,done:event.target.checked,updatedAt:nowISO()}:item);await saveSettings();renderChecklist()};row.querySelector("button").onclick=async()=>{settings.checklist=items.filter(item=>item.id!==id);await saveSettings();renderChecklist()}})
  }
  function renderSettings(){const profile=window.SOLVED_AUTH?.profile()||{mode:"guest"},connected=profile.mode==="authorized-google";els.settingsProfileName.textContent=connected?(profile.name||"Cuenta SOLved"):"Modo invitado";els.settingsProfileEmail.textContent=connected?(profile.email||""):"Datos guardados en este dispositivo";els.settingsProfileStatus.textContent=connected?"Cuenta conectada y sincronización disponible":"Sin cuenta conectada";window.LBT_UTILS?.renderWeekly()}
  function renderDashboard(){renderHandList();renderCurrent();renderWeeklySchedule();renderEvents("upcomingEvents",6);renderChecklist()}
  ...
  async function renderStudy(){
    ...
    window.LBT_CONTENT.bind(els.studyBody);const richFrame=els.studyBody.querySelector(".rich-document");if(richFrame)richFrame.addEventListener("load",()=>sendRichFrameState(richFrame));window.LBT_NOTES.render();window.LBT_UTILS?.applyReading(currentSubject?.id);
  ...
  function richFrame(){return currentTab==="summary"?els.studyBody.querySelector(".rich-document"):null}
  ...
  async function handleSummaryCommand(event){
    ...
    if(data.command==="fullscreen"){await enterFullscreen();return true}
    if(data.command==="open-index"){if(!settings.indexVisible)await toggleIndex();return true}
    if(data.command==="save-theme"&&["light","dark"].includes(data.theme)){settings.theme=data.theme;applyTheme();await saveSettings();sendRichFrameState(frame);return true}
    if(data.command==="save-color"&&Number.isFinite(Number(data.hue))){await persistSubjectHue(currentSubject.id,Math.max(0,Math.min(360,Number(data.hue))));sendRichFrameState(frame);return true}
    if(data.command==="remove-highlight"){const item=highlights.find(entry=>entry.id===data.highlightId);if(item){item.deletedAt=nowISO();item.updatedAt=item.deletedAt;await persist("highlights",item);sendRichFrameState(frame);toast("Resaltado eliminado")}return true}
    if(data.command==="highlight"&&data.selection){const piece=data.selection;if(typeof piece.blockId!=="string"||typeof piece.exact!=="string"||!piece.exact.trim()||!Number.isInteger(piece.start)||!Number.isInteger(piece.end)||piece.start<0||piece.end<=piece.start)return true;const overlap=richHighlights().some(item=>item.blockId===piece.blockId&&Math.max(item.start,piece.start)<Math.min(item.end,piece.end));if(overlap){toast("La selección toca un resaltado existente");return true}const stamp=nowISO(),item={id:uuid(),subjectId:currentSubject.id,unitId:unit?.unitId||null,studyBlock:els.studyUnit.value,tab:"summary",...piece,color:"yellow",contentVersion:unit?.contentVersion||CONTENT_VERSION,createdAt:stamp,updatedAt:stamp};highlights.push(item);await persist("highlights",item);sendRichFrameState(frame);toast("Resaltado guardado");return true}
    return false;
  }
  function updateToolbarVisibility(){const summary=currentTab==="summary",hasText=!!document.querySelector(".highlightable")||!!richFrame();els.highlightBtn.hidden=!summary||!hasText;els.indexBtn.hidden=!summary||!document.querySelector(".highlightable");els.selectionHelp.hidden=!summary||!hasText;els.indexBtn.classList.toggle("active",settings.indexVisible)}
  ...
  function applyAllHighlights(){
    if(richFrame()){sendRichFrameState();return}
  ...
  function updateDriveState(state){
    ...
    const connected=!!driveSync?.hasToken();els.driveActionBtn.hidden=state==="syncing";els.driveActionBtn.textContent=state==="reconnect"?"Reconectar Drive":connected?"Sincronizar ahora":"Conectar Google Drive";els.driveDisconnectBtn.hidden=!connected;els.syncPill.disabled=state==="syncing";els.syncPill.setAttribute("aria-label",labels[state]||state);
  }
  ...
  function bindEvents(){
    ...
    els.checklistForm.onsubmit=async e=>{e.preventDefault();const text=els.checklistInput.value.trim();if(!text)return;settings.checklist=[...(settings.checklist||[]),{id:uuid(),text,done:false,createdAt:nowISO(),updatedAt:nowISO()}];els.checklistInput.value="";await saveSettings();renderChecklist();els.checklistInput.focus()};
    ...
    window.addEventListener("message",async event=>{if(await handleSummaryCommand(event))return;const frame=richFrame();if(!frame||event.source!==frame.contentWindow)return;const data=event.data||{};if(data.type==="solved-rich-selection"&&data.piece){pendingSelection=[data.piece];activeHighlightId=null;updateHighlightButton()}if(data.type==="solved-rich-highlight-active"){activeHighlightId=data.id||null;pendingSelection=null;updateHighlightButton()}});
  ...
  function registerPWA(){
    ...
    if("serviceWorker" in navigator&&location.protocol.startsWith("http"))navigator.serviceWorker.register(`./sw.js?v=${APP_VERSION}`,{scope:"./"}).then(reg=>{if(reg.waiting)showUpdate(reg.waiting);reg.update();reg.addEventListener("updatefound",()=>{const worker=reg.installing;worker?.addEventListener("statechange",()=>{if(worker.state==="installed"&&navigator.serviceWorker.controller)showUpdate(worker)})})}).catch(e=>console.warn("Service worker",e));
