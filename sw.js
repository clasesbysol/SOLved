...
const CACHE_VERSION = "biblioteca-lbt-v01117-subject-skeleton-v3";
const CORE = [
  ...
  "./js/subject-skeleton.js?v=0.1.0","./js/integral-frame-tools.js?v=1.0.0","./js/reading-mode-v2.js?v=1.1.0","./js/qbi-official-frame-fix.js?v=1.0.0","./js/fisica-first-partial-guide.js?v=1.1.0","./js/fisica-topic-intros.js?v=1.2.0",
...
async function injectAppShell(request,fallbackUrl){const response=await cacheFirstAndRefresh(request,fallbackUrl);if(!response||!response.ok)return response||Response.error();let text=await response.text();const injections=[];if(!text.includes('data-solved-reading-v2="1"'))injections.push('<script src="./js/reading-mode-v2.js?v=1.1.0" data-solved-reading-v2="1"></script>');if(!text.includes('data-solved-qbi-frame-fix="1"'))injections.push('<script src="./js/qbi-official-frame-fix.js?v=1.0.0" data-solved-qbi-frame-fix="1"></script>');if(!text.includes('data-solved-integral-frame-tools="1"'))injections.push('<script src="./js/integral-frame-tools.js?v=1.0.0" data-solved-integral-frame-tools="1"></script>');if(!text.includes('data-solved-estadistica-integral="1"'))injections.push('<script src="./js/estadistica-integral-bridge.js?v=1.4.0" data-solved-estadistica-integral="1"></script>');if(!text.includes('data-solved-update-status="1"'))injections.push('<script src="./js/solved-update-status.js?v=1.0.0" data-solved-update-status="1"></script>');if(injections.length)text=text.replace(/<\/body>/i,`${injections.join("")}</body>`);const headers=new Headers(response.headers);headers.delete("content-length");headers.delete("content-encoding");headers.set("content-type","text/html; charset=utf-8");return new Response(text,{status:response.status,statusText:response.statusText,headers})}
