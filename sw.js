...
const CACHE_VERSION = "biblioteca-lbt-v01124-organica-nativa";
const CORE = [
  "./","./index.html","./bienvenida.html",
  "./styles.css?v=0.11.0","./styles-enhancements.css?v=0.11.16","./styles-personal.css?v=0.10.9",
  "./organic-cards.css?v=0.7.3","./organic-mind-map.css?v=0.7.4","./organic-native.css?v=1.0.0",
  "./js/supabase-config.js?v=0.8.0","./js/auth.js?v=0.8.1","./js/data.js?v=0.11.16","./js/db.js?v=0.10.9","./js/supabase-sync.js?v=0.10.9",
  "./js/organic-cards.js?v=0.7.3","./js/organic-anchor-index.js?v=1.0.0","./js/organic-unit-index.js?v=1.0.0","./js/organic-native.js?v=1.0.0","./js/qbi-mind-map-data.js?v=1.1.0","./js/qbi-mind-map-data.js?v=4.4.0","./js/organic-mind-map.js?v=0.11.1","./js/sync.js?v=0.11.0","./js/content.js?v=0.11.7","./js/study-workspace.js?v=0.11.16","./js/notes.js?v=0.7.3","./js/utilities.js?v=0.11.0","./js/app.js?v=0.11.16",
  ...
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-integrated-subject.js?v=4.7.0",
  ...
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-guide-memory-maps-more.js?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-guide-memory-equations.js?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-enzimas3-integration.js?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-enzimas3-exercise-figures-addon.js?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-tp2-extension.js?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-glucidos-integration.js?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-glucidos-force-463.js?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-glucidos-summary-1.txt?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-glucidos-summary-2.txt?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-glucidos-summary-3.txt?v=4.7.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-glucidos-summary-4.txt?v=4.7.0",
  ...
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-exercises-glucidos-part-1.json?v=1.0.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-exercises-glucidos-part-2.json?v=1.0.0",
  "./content/subjects/quimica_biologica1/units/proteinas-i/qbi-exercises-glucidos-part-3.json?v=1.0.0",
...
async function injectAppShell(request,fallbackUrl){const response=await cacheFirstAndRefresh(request,fallbackUrl);if(!response||!response.ok)return response||Response.error();let text=await response.text();const injections=[];if(!text.includes('data-solved-reading-v2="1"'))injections.push('<script src="./js/reading-mode-v2.js?v=1.1.0" data-solved-reading-v2="1"></script>');if(!text.includes('data-solved-qbi-frame-fix="1"'))injections.push('<script src="./js/qbi-official-frame-fix.js?v=1.3.0" data-solved-qbi-frame-fix="1"></script>');if(!text.includes('data-solved-qbi-glucidos-pack="1"'))injections.push('<script src="./js/qbi-glucidos-study-pack.js?v=1.0.0" data-solved-qbi-glucidos-pack="1"></script>');if(!text.includes('data-solved-integral-frame-tools="1"'))injections.push('<script src="./js/integral-frame-tools.js?v=1.0.0" data-solved-integral-frame-tools="1"></script>');if(!text.includes('data-solved-persistence-repair="1"'))injections.push('<script src="./js/persistence-repair.js?v=1.0.0" data-solved-persistence-repair="1"></script>');if(!text.includes('data-solved-estadistica-integral="1"'))injections.push('<script src="./js/estadistica-integral-bridge.js?v=1.4.0" data-solved-estadistica-integral="1"></script>');if(!text.includes('data-solved-update-status="1"'))injections.push('<script src="./js/solved-update-status.js?v=1.1.0" data-solved-update-status="1"></script>');if(!text.includes('data-solved-study-chrome="1"'))injections.push('<script src="./js/study-chrome.js?v=0.11.17" data-solved-study-chrome="1"></script>');if(!text.includes('data-solved-analysis2-integrated="1"'))injections.push('<script src="./js/analysis2-integrated.js?v=1.0.0" data-solved-analysis2-integrated="1"></script>');if(!text.includes('data-solved-developer-comments="1"'))injections.push('<script src="./js/developer-comments-v101.js?v=1.0.1" data-solved-developer-comments="1"></script>');if(injections.length)text=text.replace(/<\/body>/i,`${injections.join("")}</body>`);const headers=new Headers(response.headers);headers.delete("content-length");headers.delete("content-encoding");headers.set("content-type","text/html; charset=utf-8");return new Response(text,{status:response.status,statusText:response.statusText,headers})}
self.addEventListener("fetch",event=>{const request=event.request;if(request.method!=="GET")return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==="navigate"){const internalDocument=url.pathname.includes("/content/")||/\.(?:html?|pdf)$/i.test(url.pathname);event.respondWith(internalDocument?cacheFirstAndRefresh(request):injectAppShell(request,"./index.html"));return}if(url.pathname.endsWith("/content/catalog.json")||url.pathname.endsWith("/version.json")){event.respondWith(fetch(request,{cache:"no-store"}));return}if(url.pathname.endsWith("/content/subjects/fisica1/units/resumen-integral/original.html")){event.respondWith(injectFisicaGuides(request));return}if(["script","style","worker"].includes(request.destination)){event.respondWith(cacheFirstAndRefresh(request));return}event.respondWith(staleWhileRevalidate(request))});
